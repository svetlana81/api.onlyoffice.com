...
| onItemClick   | func                                                                                                       | `(id: string \| number) => {}`                                    | Optional. Defines a function that is invoked when the main button action is triggered. Takes the folder/room id as an argument. Supports both string and number identifiers. This function can be asynchronous and can return `IMessage`, `Promise<IMessage>`, or `Promise<void>`.                                                                                                                                                         |
| onClick       | func                                                                                                       | `(id: number) => {}`                                              | Optional. **Deprecated.** Use `onItemClick` instead. Defines a function that takes the folder/room id as an argument. This function can be asynchronous.                                                                                                                                                           |
...
| items         | IMainButtonItem[]                                                                                   | `[createItem]`                                                    | Optional. Defines the main button items that are added to the current item as a drop-down list. In this case, the [onItemClick](#onitemclick) event does not work.                                                                                                                                                  |
